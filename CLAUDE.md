...
- **The copy under the headline waits for it.** All three lines hold at `opacity 0` until `HEADLINE_SETTLES` (2.6s — when StrokeText's outline and wipe have both landed), then rise in on three beats 0.14s apart so they arrive in reading order: **disclosure, promise, detail**. The delay collapses to 0 under reduced motion, because StrokeText jumps to its end state there and the value proposition would otherwise sit invisible waiting on an animation that already finished. `MotionConfig reducedMotion="user"` drops the transform by itself but has no opinion about delays.
- **The demo chip is not a second `DemoMarker`.** `AppHeader`'s version is a solid accent pill because it has to survive a dense bar; two of those on one screen read as a warning banner. The hero's is the same information at a tenth of the volume — accent reduced to a 6px dot, words in `text-muted`, hairline border over `bg-page/50`. It also **states the disclosure outright instead of hiding it in a tooltip**, which the header's version does: hover never fires on touch, so on a phone the header's disclosure does not exist. `TeamsHero.test.tsx` pins both that it is visible text and that it sits *above* the value proposition in DOM order — a visitor must not read "the easy, transparent and secure way to buy" before learning the prices are invented.
